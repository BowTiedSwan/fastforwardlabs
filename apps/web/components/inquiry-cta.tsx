...
export function InquiryCta({ slug, name }: { slug: string; name: string }) {
  return (
    <section className="grid gap-8 border border-line bg-panel p-6 sm:p-9 md:grid-cols-[1.2fr_0.8fr] md:p-12">
      ...
      <div className="flex items-center md:border-l md:border-line md:pl-10"><Button asChild size="lg"><Link href={`/contact?service=${slug}`}>Discuss your project <ArrowUpRight aria-hidden="true" className="size-4" /></Link></Button></div>
    </section>
  );
}
